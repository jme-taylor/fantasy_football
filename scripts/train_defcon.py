"""Train and evaluate the defcon rate head on its own.

    uv run python scripts/train_defcon.py
...
def main() -> None:
    """Train the defcon rate head and log the run to MLflow."""
    configure_logging()
    connection = get_connection(DATABASE_PATH)
    try:
        predictor = DefconRatePredictor(
        ...
        logger.info(
            ...
            DEFCON_SPEC.registered_model_name,
        )
    finally:
        connection.close()
